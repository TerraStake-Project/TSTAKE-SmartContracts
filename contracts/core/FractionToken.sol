    ...
    UUPSUpgradeable
{
    ...
    ITerraStakeNFT.NFTProjectCategory public projectCategory;
    ...
    function initialize(
        ...
        ITerraStakeNFT.NFTProjectCategory _projectCategory,
    ...
    ITerraStakeFractionManager
{
    ...
    function initialize(
        ...
        __Pausable_init();
        __UUPSUpgradeable_init();
        _setChainlinkToken(0xf97f4df75117a78c1A5a0DBb814Af92458539FB4); // LINK token on Arbitrum
    ...
    function requestImpactDataUpdate(address fractionToken) public onlyRole(VERIFIER_ROLE) returns (bytes32) {
        if (projectsContract == address(0)) revert ProjectsContractNotSet();
        FractionData storage data = fractionData[fractionToken];
        if (!data.isActive) revert FractionalizationNotActive();


        Chainlink.Request memory request = _buildChainlinkRequest(chainlinkJobId, address(this), this.fulfill.selector);
        Chainlink._add(request, "projectId", uintToString(data.projectId));
        Chainlink._add(request, "reportId", uintToString(data.reportId));
        bytes32 requestId = _sendChainlinkRequestTo(chainlinkOracle, request, chainlinkFee);
    ...
    function _fractionalizeNFT(FractionParams memory params, bool isProjectNFT) internal returns (address) {
        ...
        (uint256 impactValue, ITerraStakeNFT.NFTProjectCategory category, bytes32 verificationHash) = terraStakeNFT.getTokenData(params.tokenId);
