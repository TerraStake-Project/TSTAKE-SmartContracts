...
/**
 * @dev Cancels a governance proposal.
 * @param proposalId The ID of the proposal.
 */
function cancelProposal(bytes32 proposalId) external whenNotPaused {
    Proposal storage proposal = proposals[proposalId];
    require(proposal.proposalHash != bytes32(0), "Proposal does not exist");
    require(!proposal.executed, "Proposal already executed");
    require(msg.sender == proposal.creator || hasRole(GOVERNANCE_ROLE, msg.sender), "Not authorized");
    ...
    whenNotPaused 
{
    ...
    // Create new fraction token using the initialize pattern for upgradeable contracts
